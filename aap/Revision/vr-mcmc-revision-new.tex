...
In the next section   we provide a detailed   analysis of  $\mathsf{Var}[\pi_{K,n}^N(f)]$  for the  ULA  algorithm (see Example~\ref{exam:langevin-algorithm}). In many situations the coefficients $\bar{a}_{r,k}$ also decay exponentially fast as $r$ increases. The following result can be easily established.
\begin{lem}
...
\end{equation}
where $\rho\in (0,1)$ and $C$ is an absolute constant. In particular, if $f$ is uniformly bounded we can take $V(x)\equiv 1$ to get
\begin{eqnarray*}
|\bar a_{r,k}(x)| \leq C\rho^r\|f\|_{\infty}.
\end{eqnarray*}

\end{lem}
This lemma shows that in fact  only a logarithmic number (in $n$) of coefficients need to be computed.

\section{Estimation of coefficients and variance reduction}
...
We present a generic regression algorithm. Algorithm starts with estimating the functions 
\[
S_{q}(x)=\mathsf{E}\left[\sum_{r=0}^q f(X^x_{r})\right]
\]
for $q=N+1,\ldots,N+n.$ We first generate \(\NtrainPath\)  paths  of the chain \(X\)
...
\widehat{S}_q = \argmin_{\psi \in \Psi} \sum_{s = 1}^{T} \sum_{l=N+1}^{L}\left| \sum_{r = 0}^{q}  f(X_{r+l}^{(s)}) - \psi(X^{(s)}_l)\right|^2
\end{equation}
for some $L$ with $L+q\leq N+n,$
where \(\Psi\) is a  class of  real-valued functions on \(\mathbb{R}^d\) and set  $\widehat{S}_{R,q}=\max\{\widehat{S}_q,R\}$ for some $R>0.$
Next  we estimate the coefficients \( A_{q,k}\) using
the formula
\begin{equation}
\label{eq:a_est_int}
\widehat A_{q,k}(x)=\mathsf{E}\left[\left.\phi_k\left(\xi\right)\widehat S_{R,q}\left(\Phi(x,\xi)\right)\right | \TrainSet\right] \,,
\end{equation}
where $\xi$ is a random variable independent from $\mathcal{T}$ with law $P_\xi,$
$\Phi$ is defined in \eqref{eq:chain_gen} and the expectation is taken according to known distribution \(P_\xi\). Conditioning on $\TrainSet$ underlines that estimation of $\widehat S_{R,q}$ derived from training trajectories. In many cases integration can be done in closed analytical form. For example, in the case of ULA algorithm we have \(P_\xi$ is the multivariate normal distribution $ \mathcal{N}(0,I_d)\) and \(\Phi(x,\xi)=x-\gamma\mu(x)+\sqrt{\gamma}\xi.\) Therefore if we use polynomials to approximate \((S_{q}),\) then \((\widehat A_{q,k})\) can be even computed in closed form; implementations details are provided in  Section~\ref{sec:num}.
Upon estimating  the functions \((\widehat A_{q,k})\), one can construct an empirical estimate  of \(M_{K,n}^N(f)\) in the form
\begin{eqnarray*}
...
Suppose that $\|f\|_\infty\leq F$ and $\|A_{l,k}\|_{\infty}\leq R$ for all $k=1,\ldots,K,$ $l=1,2,\ldots $ and some constants $F,K>0.$ The standard regression estimates imply that 
\begin{eqnarray*}
\PE\left[\left.|A_{n-l,k}(X^x_{N+l-1})-\widehat A_{n-l,k}(X^x_{N+l-1})|^2\right | \TrainSet \right]&\lesssim &  \frac{(R^2 \vee F^2)\,\mathrm{Dim}(\Psi)}{T}
\\
&&+\sup_{\psi\in\Psi}\|A_{n-l,k}-\psi\|_{L^2(P^x_{N+l-1})}
\end{eqnarray*}
...
\begin{equation}
\label{eq:sq_bias}
\left( \mathsf{E}[\rho(f)]-\pi(f)\right)^2 \lesssim \|f\|^2_{\infty}\left(\gamma + \frac{\rho^{2N}}{n(1-\rho^2)}\right) \lesssim \|f\|^2_{\infty}\left(\gamma + \frac{e^{-\gamma \kappa N}}{2\gamma n}\right)
\end{equation}
for $V(x) = 1 + \|x\|^2$, $\rho$ defined in Lemma~\ref{lem:v_ergodicity}, $\kappa$ independent of $N,n,\gamma$ and $\lesssim$ means less up to a constant not depending on $N,n$ and $\gamma$.
\end{lem}

\begin{proof} Under assumptions {\bf (H1)} and {\bf (H2)}, the corresponding Markov chain has a unique stationary distribution $\pi_\gamma$, which is different from $\pi$. Thus we represent the squared bias as 
\begin{eqnarray}
\label{eq:bias_1}
\nonumber
...
=
\frac{1}{n^2}\left[\sum\limits_{k=N+1}^{N+k}\left(f(X_k^x)-\pi(f)\right)\right]^2 \\
&\leq& 
2|\pi_{\gamma}(f) - \pi(f)|^2 + \frac{2}{n}\sum\limits_{k=N+1}^{N+n}\left[\PE f(X_k^x) - \pi_{\gamma}(f)\right]^2
\end{eqnarray}
 From \cite[Theorem~10]{durmus:moulines:2017} and Lemma~\ref{lem:v_ergodicity} it follows that
\begin{equation}
\label{eq:bias_2}
\tvnorm{\pi - \pi_\gamma} < C_1\sqrt{\gamma}, \quad |\PE f(X_k^x) - \pi_{\gamma}(f)| \leq C_2 \rho^n\|f\|_{V}V(x)
\end{equation}
for  $V(x) = 1 + \|x\|^2,$ $\rho\in (0,1)$ specified in Lemma~\ref{lem:v_ergodicity} and constants $C_1,C_2>0$ independent of $\gamma$  and $n$. Since $f$ is bounded, $\|f\|_V \leq \|f\|_{\infty}$. Hence, combining \eqref{eq:bias_1} and \eqref{eq:bias_2}, we obtain \eqref{eq:sq_bias}.
\end{proof}
...
Note that the bound of lemma~\ref{lem:variance} is sharp, namely, the following statement holds:
\begin{lem} 
\label{lem:ula_var_lower_bound}
Let $f(x) = x$ and $(X^x_{p})_{p \in \nset_0}$ be a Markov chain generated by ULA with constant step size $\gamma$, with $\pi(x) = \frac{1}{\sqrt{2\pi}}e^{-\frac{x^2}{2}}$ and assume also $n\gamma > C_0$ for some fixed $C_0$. Then it holds
\begin{equation}
\label{eq:lower_bound}
\PVar[\pi_n^0(f)] \geq \frac{2}{n\gamma} - \mathcal{O}\left(\frac{1}{n^2\gamma^2}\right) \eqsp,
\end{equation}
\end{lem}
...
\begin{eqnarray}
\label{eq:var-bound}
\PVar\left(\pi_{K,n}^{N}(f)\right)\lesssim n^{-1} \gamma^{K-1} 
\eqsp,
\end{eqnarray}
...
\begin{eqnarray*}
\sum_{\mathbf{k}\colon\|\mathbf{k}\|\geq K} A^2_{q,\mathbf{k}}(x)\leq \sum_{I\subseteq\{1,\ldots,d\},\, I\neq \emptyset}
...
\begin{eqnarray*}
\sum_{\mathbf{k}\colon\|\mathbf{k}\|\geq K}A^2_{s,\mathbf{k}}(x)
...
\section{Proofs}
\subsection{Proof of Lemma~\ref{lem:coef_decrease}}
Note that 
\[
\bar a_{r,k}(x)=\mathsf{E}\left[\phi_k\left(\xi\right)Q_{r}\left(\Phi(x,\xi)\right)\right]
\]
...
\[
|Q_{r}(x) - \pi(f)| \leq \left|\int\limits_{\rset^d} f(y)\left(\P^r(x,dy)-\pi(dy)\right)\right| \leq C\rho^r V(x) \|f\|_{V}
\]
...
\subsection{Proof of Lemma~\ref{lem:ula_var_lower_bound}}
Note that ULA for the standard normal distribution takes form
\begin{eqnarray*}
X_{k+1} = (1-\gamma)X_k + \sqrt{2\gamma}\xi_{k+1},\, \xi_{k+1} \sim \mathcal{N}(0,1)
\end{eqnarray*}
Thus, setting $X_0 = 0$, we obtain
\begin{eqnarray*}
\PVar_{0} \pi_n^0(f) &=& \PVar_{0}\left[\frac{1}{n}\sum\limits_{i=1}^{n}X_i\right] = \frac{1}{n^2}\PVar\left[\sum\limits_{i=1}^{n}\sum\limits_{j=1}^{i}\sqrt{2\gamma}(1-\gamma)^{i-j}\xi_{j}\right] = \frac{1}{n^2}\PVar\left[\sum\limits_{j=1}^{n}\xi_j\frac{\sqrt{2\gamma}(1-(1-\gamma)^{n-j+1}}{\gamma}\right] = \\
&=&\frac{2}{n^2\gamma}\sum\limits_{j=1}^{n}\left(1-(1-\gamma)^{n-j+1}\right)^2 = \frac{2}{n\gamma} - \frac{4(1-\gamma)\left[1-(1-\gamma)^{n+1}\right]}{n^2\gamma^2} + \frac{2(1-\gamma)^2\left[1-(1-\gamma)^{2n+2}\right]}{n^2\gamma^2(2-\gamma)} \geq \\
& \geq & \frac{2}{n\gamma} - \frac{4(1-\gamma)}{n^2\gamma^2} \geq \frac{2}{n\gamma} - \frac{4}{n^2\gamma^2}
\end{eqnarray*}
