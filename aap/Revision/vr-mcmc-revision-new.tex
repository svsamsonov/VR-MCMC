...
\]
$\psi_n(y) < \mathsf{E}[|f(X^y_{q,q+1}) - \mathsf{E} f(X^y_{q,q+1})|^2] \leq \frac{\|f\|^2_{\infty}}{4}$. Hence, by Lebesgue dominated convergence theorem, $\mathsf{E}\psi^2_n(X_q^x) \rightarrow 0$ and since $\mathsf{E}[f(X^{X^x_q}_{q,q+1})|X^x_{q}] = \mathsf{E}[f(X^{x}_{q+1})|X^x_{q}]$, it holds in \(L^2(\mathsf{P})\) that
\begin{equation}
