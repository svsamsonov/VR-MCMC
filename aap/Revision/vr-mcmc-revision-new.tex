...
\begin{eqnarray}
\label{eq:sde-inv}
dX_t=b(X_t)\, \rmd t+\sigma(X_t)\, \rmd W_t,\quad t\geq 0,	
\end{eqnarray}
...
$\mathsf{E}\left[\left|f(X_{p})\right|^{2}\right]<\infty$,   where $(X_{p})_{p\geq 0}$ is a Markov chain of the form \eqref{eq:chain_gen} for some kernels $(\Phi_l)_{l\geq 1}.$
Then,
...
\]
Since \(X_{l-1}\) is independent of \(\xi_{l}\) and \(\mathsf{E}[\phi_k(\xi_{l})]=0,\) \(k\neq l,\)  the r.v.  \(M_{K,n}^N(f)\) has zero mean and can be viewed as a control variate.
...
\end{equation}
should be small for \(K\) large enough. Indeed, due to orthonormality  we have
\[
...
\begin{eqnarray*}
\langle D^2U(x),x\rangle \geq \frac{m_U}{2}\|x\|^2.
\end{eqnarray*}
\end{description}
...
for $\gamma>0$, the Markov kernel $Q_\gamma$
is defined by
$$
\mathcal{Q}_\gamma(x,A)=\int_{A} \frac{1}{(4 \pi \gamma)^{d/2}} \text{ exp} \left\{ -\frac{1}{4\gamma} \left\| y - x + \gamma \mu(x)\right\|^2\right\}dy.
$$
...
on bounds in the total variation distance. It is known  that under assumptions {\bf (H1)} and {\bf (H2)}  the corresponding Markov chain has a unique stationary distribution $\pi_\gamma$, which is different from $\pi$. Moreover, as shown in Proposition (Appendix) \begin{eqnarray}
\label{eq:bias}
\|\delta_x\mathcal{Q}_{\gamma}^{n}-\pi_\gamma\|_{TV} \leq C_1\rho^{\gamma n}\left(V(x) + \pi_\gamma(V)\right), \quad \|\pi-\pi_\gamma\|_{TV} \leq C_2 \sqrt{\gamma}
\end{eqnarray}
...
\begin{eqnarray}
\label{eq:var-mc}
\mathsf{Var}[\pi_n^N(f)]\lesssim \frac{1}{n\gamma},
\end{eqnarray}
...
\begin{eqnarray}
\label{eq:smooth-mu}
|\partial^{\mathbf{k}} f(x)|\le  B_f, \quad |\partial^{\mathbf{k}} \mu (x)|\leq B_\mu, \quad
x\in\mathbb R^d
\end{eqnarray}
for all \(\mathbf{k}\) with \(0<\|\mathbf{k}\|\leq  K.\)  Then it holds
\begin{eqnarray}
\label{eq:var-bound}
\mathsf{Var}\left[\pi_{K,n}^{N}(f)\right]\lesssim \frac{\gamma^{K-2}}{n},
\end{eqnarray}
where \(\lesssim\) stands for inequality up to a constant not depending  on $\gamma,$ \(n\) and \(N.\)
\end{thm}
...
\end{eqnarray}
where $F_s(X_0,Z_1,\ldots,Z_s)=\frac{1}{s}\sum_{r=0}^s f(X_r)$ and $\partial_{Z_1}^{\mathbf{k}'}$
...
\mathsf{Var}_x\left(\frac{1}{s}\sum_{p=1}^{s}\partial_{y_{1}}^{\mathbf{K}_I}f\left(X_{p}\right)
\right),
\end{eqnarray*}
where the sum runs over all nonempty subsets $I$ of the set $\{1,\ldots,d\}$ and
\[
\mathbf{\mathbf{K}}_{I}=(K1_{\{1\in I\}},\ldots,K1_{\{d\in I\}}).
\]
