...
$\phi_k(\xi)$, $k\ge1$, are centered. As an example, we can take  multivariate Hermite polynomials for the ULA algorithm and a tensor product of Shifted Legendre polynomials for "uniform part" and Hermite polynomials for "Gaussian part"  of the random variable $\xi = (u, z)^T$ in MALA, as the Shifted Legendre polynomials are orthogonal with respect to the Lebesgue measure on \([0,1].\) We shall use the notations $\nset=\{1,2,\ldots\}$ and $\nset_0=\mathbb N\cup\{0\}$. We denote by $(\mathcal{G}_p)_{p\in \mathbb{Z}_+}$  the filtration generated by independent random variables $(\xi_p)_{p \in \mathbb{N}^*}$ with the convention $\mathcal{G}_0=\mathrm{triv}$. For $l \le p$ and $x \in \rset^d$, put for $x,y \in \rset^d$ functions $\Phi_k(x,y)=x-\gamma_k\mu(x)+y$. Then we can define
\[
X^x_{l,p}=G_{l,p}(x,\sqrt{\gamma_l}\xi_{l},\ldots,\sqrt{\gamma_p}\xi_{p}),
\]
with the function \(G_{l,p}:\) \(\mathbb{R}^{d\times(p-l+2)}\to \mathbb{R}^{d}\) is defined as
\begin{equation}
\label{eq:definition-G-p-l}
G_{l,p}(x,y_l,\ldots,y_p):=\Phi_p(\cdot,y_{p})\circ\Phi_{p-1}(\cdot,y_{p-1})\circ\ldots\circ\Phi_{l}(x,y_{l})
\end{equation}
For simplicity we will write $X^x_{p}$ and $G_{p}$ instead of $X^x_{0,p}$ and $G_{0,p}$. As a consequence, for a function $f\colon \rset^d \to \rset$ we have
$$
f\left(X^x_{p}\right) =%f\circ G_{p}(x,\sqrt{\gamma}Z_{1},\ldots,\sqrt{\gamma}Z_{p}).
f\circ G_{p}(x,\sqrt{\gamma}\xi_{1},\ldots,\sqrt{\gamma}\xi_{p})
$$
...
\begin{thm}\label{prop:29032018a1}
 Let $f$ be a Borel function $\rset^{d}\to\mathbb R$ such that
...
a_{p,l,k}(x)=\mathsf{E}\left[f(X^x_{l-1,p})\phi_k\left(\xi_{l}\right)\right], \quad p\geq l, \quad k\in \mathbb{N}
\end{eqnarray}
\end{thm}

\begin{proof}%[Proof of Theorem~\ref{prop:29032018a1}]
The expansion obviously holds for $p=1$ and $j=0$.
Indeed, since $\left(\phi_{k}\right)_{k \geq 0}$ is a complete orthonormal system in \(L^2(\mathbb{R}^d, P_{\xi})\), we have for all $x \in \rset^d$,
\eric{it is clear, but we must say in which sense this equality should be understood}
\[
f(X_{1})=\PE[f(X_{1})]+\sum_{k\geq1}a_{1,1,k}(x)\phi_{k}(\xi_{1})
\]
with $a_{1,1,k}(x)=\PE[f(X_{1}^x)\phi_{k}(\xi_{1})]$,
provided $\PE[|f(X_{1})|^{2}]<\infty.$
...
with
\begin{eqnarray*}
\bar a_{r,k}(x)=\mathsf{E}\left[f(X^x_{r})\phi_k\left(\xi_{1}\right)\right], \quad r, k\in \mathbb{N}.
\end{eqnarray*}
\end{cor}
...
\[\mathsf{E}[A_{N+n-l,k}(X_{l-1})\phi_{k}\left(\xi_{l}\right)] = \mathsf{E}[A_{N+n-l,k}(X_{l-1})\mathsf{E}[\left.\phi_{k}\left(\xi_{l}\right)\right|\mathcal{G}_{l-1}]] = 0\]
\eric{I do not understand this} and hence the r.v.  \(M_{K,n}^N(f)\) has zero mean and can be viewed as a control variate.
...
\end{eqnarray}
for \(1 \leq r \leq n-1,\), $\Psi$ being the class of polynomials $\Psi = \{\psi(x) | \psi(x) = \sum_{\|\mathbf{s}\|\leq m} \alpha_{\mathbf{s}} x^{\mathbf{s}} \}$ and $\widehat{Q}_0(x) = f(x)$ by definition. Due to Lemma~\ref{a_decay},
\sergey{we need to put some lemma about the truncation point here, I will do it}
it is enough to estimate $Q_{r}$ for $r < n_{\mathrm{trunc}}$ for some truncation level \(n_{\mathrm{trunc}}\) depending on \(d\) and \(\gamma.\) It allows us to use a smaller amount of training trajectories to approximate $Q_{r}$.


Finally  we construct a truncated version of the estimator from \ref{}:
\[
\widehat\pi_{K,n,n_{\mathrm{trunc}}}^N(f) = \widehat\pi_n^N(f) -  \widehat M_{K,n, n_{\mathrm{trunc}}}^{N}(f),
\]
where
\begin{eqnarray*}
\widehat M_{K,n,n_{\mathrm{trunc}}}^N(f) &=&\frac{1}{n}\sum_{p=N+1}^{N+n}\left[\sum_{k=1}^{K}\sum_{l=N+1}^{p} \bar a_{p-l,k}(X_{l-1})\phi_k(\xi_{l})\right]
\end{eqnarray*}
and
\begin{eqnarray*}
