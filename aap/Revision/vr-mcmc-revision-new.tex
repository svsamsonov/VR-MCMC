...
Let \((X^x_{p})_{p\geq 0}\) be a Markov chain of the form \eqref{eq:ula_new}. Then, for $p>j$ and for all Borel functions $f: \rset^{d}\to \rset$ such that $\mathsf{E}\left[\left |f(X^x_{p})\right|^{2}\right]<\infty$, the following representation holds in \(L^2(\mathsf{P})\)
\begin{eqnarray}
\label{eq:mart_repr}
f(X^x_{p})=\mathsf{E}\left[\left.f(X^x_{p})\right|\mathcal G_{j}\right]+\sum_{k=1}^{\infty}\sum_{l=j+1}^{p}a_{p,l,k}(X^x_{l-1})\phi_k\left(\xi_{l}\right),
\end{eqnarray}
...
a_{p,l,k}(y)=\mathsf{E}\left[f(X^y_{l-1,p})\phi_k\left(\xi_{l}\right)\right], \quad p\geq l, \quad k\in \mathbb{N}
\end{eqnarray}
\end{thm}
...
Assume now that (\ref{eq:mart_repr})
holds for $p=q$, all $j<q$, and Borel-measurable functions
$f$ with $\mathsf{E}\left[|f(X^x_{q})|^2\right]<\infty$.
Let us prove that the induction assumption holds for $p=q+1$.
Given $f$ with $\mathsf{E}\left[|f(X^x_{q+1})|^2\right]<\infty$,
due to the orthonormality and completeness
of the system $\left(\phi_{k}\right)$, we get that for any $y \in \rset^d$ it holds in \(L^2(\mathsf{P})\) 
\[
f(X^y_{q,q+1}) = \mathsf{E} f(X^y_{q,q+1}) + \sum\limits_{k=1}^{\infty}\mathsf{E}[f(X^y_{q,q+1})\phi_{k}(\xi_{q+1})]\phi_k(\xi_{q+1})
\]
which means that
\begin{equation}
\label{eq:parseval}
\lim_{n \rightarrow \infty}\mathsf{E}[|f(X^y_{q,q+1}) - \mathsf{E} f(X^y_{q,q+1}) - \sum\limits_{k=1}^{n}\mathsf{E}[f(X^y_{q,q+1})\phi_{k}(\xi_{q+1})]\phi_k(\xi_{q+1})|^2] = 0
\end{equation}
Let us define sequence of functions
\[
\psi_{q+1,n}(y) = \mathsf{E}[|f(X^y_{q,q+1}) - \mathsf{E} f(X^y_{q,q+1})|^2] - \sum\limits_{k=1}^{n}\mathsf{E}^2[f(X^y_{q,q+1})\phi_k(\xi_{q+1})].
\]
which coincide with the left side of \ref{eq:parseval}. Due to Parseval identity $\Psi_{q+1,n}(y) \rightarrow 0, n \rightarrow \infty$ for any $y \in \rset^d$ and $\psi_n(y) < \mathsf{E}[|f(X^y_{q,q+1}) - \mathsf{E} f(X^y_{q,q+1})|^2] \leq \frac{\|f\|^2_{\infty}}{4}$. Hence, by Lebesgue dominated convergence theorem, $\mathsf{E}\psi^2_n(X_q^x) \rightarrow 0$ and since $\mathsf{E}[f(X^y_{q,q+1})]$ is a version of the conditional expectation $\mathsf{E}[f(X^x_{q+1}|X^x_q = y]$, it holds in \(L^2(\mathsf{P})\) that
\begin{equation}
\label{eq:1-step-decomp}
f(X^x_{q+1}) = \mathsf{E}[f(X^x_{q+1})|X^x_{q+1}] + \sum\limits_{k=1}^{\infty}a_{q+1,q+1,k}(X^x_{q})\phi_{k}(\xi_{q+1})
\end{equation}
where
\[
a_{q+1,q+1,k}(y) = \mathsf{E}\left[f(X^y_{q,q+1})\phi_k\left(\xi_{q+1}\right)\right]
\]
%we get by
%conditioning on $\mathcal{G}_{q}$,
%\[
%f(X^x_{q+1})=\mathsf{E}\left[\left.f(X^x_{q+1})\right|{\mathcal{G}}_q\right]+\sum_{k\geq1}\alpha_{q+1,q+1,k}\phi_{k}(\xi_{q+1}),
%\]
%where
%\begin{equation*}
%\alpha_{q+1,q+1,k}
%=\mathsf{E}\left[\left.f(X^x_{q+1})\phi_{k}(\xi_{q+1})\right|\mathcal G_q\right].
%\end{equation*}
%By the Markov property of $(X^x_{l})$, it holds $\mathsf{E}[f(X^x_{q+1})|\mathcal{G}_q] = \mathsf{E}[f(X^x_{q+1})| X_q^x]$. Using Markov property together with conditioning on $\mathcal{G}_{q+1}$, we obtain
%\[
%\alpha_{q+1,q+1,k}=\mathsf{E}\left[\left.f(X^x_{q+1})\phi_{k}(\xi_{q+1})\right|X^x_{q}\right]
%=a_{q+1,q+1,k}(X^x_{q})
%\]
%where for $y \in \rset^d$
%\[
...
%\begin{align}
%\label{berm:sig_X}
%f(X^x_{q+1})=\mathsf{E}\left[\left.f(X^x_{q+1})\right|X^x_{q}\right]+\sum_{k\geq1}a_{q+1,q+1,k}(X^x_{q})\phi_{k}(\xi_{q+1}),
%\end{align}
which is the required statement in the case $j=q$.
Now assume $j<q$.
The random variable
$\mathsf{E}\left[\left.f(X^x_{q+1})\right|X^x_{q}\right]$
is square integrable and has the form
$g(X^x_{q})$,
hence the induction hypothesis applies, and we get
\begin{equation}\label{eq:28082017a1}
\mathsf{E}\left[\left.f(X^x_{q+1})\right|X^x_{q}\right]=\mathsf{E}\left[\left.f(X^x_{q+1})\right|X^x_{j}\right]+\sum_{k\geq1}\sum_{l=j+1}^{q}a_{q+1,l,k}(X^x_{l-1})\phi_{k}(\xi_{l})
\end{equation}
with
\begin{align*}
a_{q+1,l,k}(X^x_{l-1}) &= \mathsf{E}\left[\left.\mathsf{E}\left[\left.f(X^x_{q+1})\right|\mathcal{G}_{q}\right]\phi_{k}(\xi_{l})\right|\mathcal{G}_{l-1}\right]
 = \mathsf{E}\left[\left.f(X^x_{q+1})\phi_{k}(\xi_{l})\right|X^x_{l-1}\right].
\end{align*}
where for $y \in \rset^d$,
\[
a_{q+1,l,k}(y) = \mathsf{E}\left[f(X^y_{l-1,q+1})\phi_{k}(\xi_{l})\right]
\]
Formulas \eqref{eq:1-step-decomp}
and~\eqref{eq:28082017a1} conclude the induction step for $p = q+1$ and hence the proof.
\end{proof}

\begin{cor}
If  all the kernels $\Phi_l,$ $l\geq 1,$ are equal,  then  the representation \eqref{eq:mart_repr} takes the form
\begin{eqnarray*}
f(X^x_{p})=\mathsf{E}\left[\left.f(X^x_{p})\right|\mathcal G_{j}\right]+\sum_{k=1}^{\infty}\sum_{l=j+1}^{p}\bar a_{p-l,k}(X^x_{l-1})\phi_k\left(\xi_{l}\right)
\end{eqnarray*}
where for $y \in \rset^d$
\begin{eqnarray*}
...
\end{eqnarray*}
with \(Q_{p,l}(x)=\mathsf{E}\left[f(X^x_{l,p})\right],\) \(p\geq l.\)
...
\begin{eqnarray}
\label{eq:qpl}
Q_{p,l}(x)=\mathsf{E}\left[Q_{p,l+1}(X^x_{l+1})\right].
...
\\
&=& \sum_{k=1}^{K}\sum_{l=N+1}^{N+n}\left(1+\frac{N-l}{n}\right)A_{N+n-l,k}(X^x_{l-1})\phi_{k}\left(\xi_{l}\right),
\label{eq:29032018a5}
\end{eqnarray}
where
\begin{equation}
\label{eq:definition-A-s,k}
A_{s,k}(x)=\frac{1}{s}\sum_{r=1}^{s}\bar a_{r,k}(x).
\end{equation}
Since \(X^x_{l-1}\) is independent of \(\xi_{l}\) and \tcr{\(\mathsf{E}[\phi_k(\xi_{l})]=0,\) \(k\neq 0\)}, we obtain
\[
\PE[A_{N+n-l,k}(X^x_{l-1})\phi_{k}\left(\xi_{l}\right)] = \mathsf{E}[A_{N+n-l,k}(X^x_{l-1})\CPE{\phi_{k}\left(\xi_{l}\right)}{\mathcal{G}_{l-1}]} = 0
\]
and hence the r.v.  \(M_{K,n}^N(f)\) has zero mean and can be viewed as a control variate.
...
\PVar[\pi_{K,n}^N(f)]\leq\sum_{k=K+1}^{\infty}\sum_{l=1}^{n}\PE[A^2_{n-l,k}(X^x_{N+l-1})]
\end{equation}
...
In this section we perform the convergence analysis of the ULA algorithm. For the sake of clarity and notational simplicity we restrict our attention to the constant time step, that is, we take $\gamma_k =\gamma$ for any $k \in \nset$.

\sergey{N and N0 were defined here before}
\eric{these notations are used before; it is best to put all the notations in a short paragraph just after the introduction}
...
\]
\sergey{Changed notation here, put variance of the noise equal to $\gamma$}
...
\sergey{removed repeating explanation of $\lesssim$}
...
\label{sec:num}
We will perform numerical experiments based on ULA with constant step size $\gamma$. Suppose that each function  \(Q_{r}(x)\) from \ref{eq:definition-bar-a} can be well approximated by polynomials, and we constructed a polynomial approximation  for each \(Q_r(x)\)  in the form:
\begin{equation*}
\widehat{Q}_{r}(x) = \sum_{\|\mathbf{s}\|\leq m} \beta_{\mathbf{s}} x^{\mathbf{s}},\quad s=(s_1,\ldots,s_d)
\end{equation*}
...
\]
we obtain closed-form expression for the estimates $\widehat a_{r,k}(x)$ of functions $\bar a_{r,k}(x)$ from \ref{eq:definition-bar-a}. Namely, for all $x \in \mathbb{R}^d$,
\begin{eqnarray*}
...
=
\left\{\PE\left[\frac{1}{s}\sum_{p=1}^{s}\left\{\partial_{1}^{\mathbf r}f\left(X^x_{p}\right)-\PE\left[\partial_{1}^{\mathbf r}f\left(X^x_{p}\right)\right]\right\}\mathbf{H}_{\mathbf{q}}(Z_{1})\right]\right\}^{2}.
\end{equation*}
...
\left(\frac{\gamma}{2}\right)^{|I|K}
\PVar\left(s^{-1}\sum\nolimits_{p=1}^{s}\partial_{1}^{\mathbf{K}_I}f\left(X^x_{p}\right)
\right)
\end{eqnarray*}
