...
\end{equation}
where $\lesssim$ means less up to a constant not depending on $n$ and $\gamma$.
\end{lem}
...
\end{equation}
for  $V(x) = 1 + \|x\|^2,$ some $\rho\in (0,1)$ and constants $C_1,C_2>0$ independent of $\gamma$  and $n$. Combining \ref{eq:bias} with the triangle inequality, we obtain \ref{eq:sq_bias}.
\end{proof}
...
\end{eqnarray}
where $F_s(x,Z_1,\ldots,Z_s)=s^{-1}\sum_{r=0}^s f(X^x_r)$ and $\partial_{Z_1}^{\mathbf{k}'}$
...
\PVar \left( s^{-1} \sum\nolimits_{p=1}^{s}\partial_{Z_{1}}^{\mathbf{K}_I}f\left(X^x_{p}\right)
\right),
\end{eqnarray*}
...
\]
 ...
 \eric{Since we do not state was $W_s$ is, it is a bit difficult to guess why we are using a Poincare inequality there}
\begin{equation*}
\PVar \left(s^{-1} \sum\nolimits_{p=1}^{s}\partial_{Z_{1}}^{\mathbf{K}_I}f\left(X^x_{p}\right)
\right)\leq W_s(x)
\end{equation*}
...
\eric{Speak about truncation here.}
...
where we denote
\[
\zeta_{l,j}(u)=\mathsf{E}\left[e^{-u\gamma\sum_{k=l}^{j}\mu'(X_{k-1})}\right],\quad u>0.
\]
...
\[
\mathsf{E}\left[e^{-s\gamma\sum_{k=l}^{j}\mu'(X_{k-1})}\right]\leq e^{s^2\varkappa \gamma^2(j-l+1)}e^{-s\gamma (j-l+1)\pi_\gamma(\mu')}.
\]
...
yielding the bound
\[
\zeta_{l,j}(s) \leq e^{s^2\kappa (j-l+1) \gamma^2}e^{-s\gamma(j-l+1)\alpha}
\]
where $ \alpha = \frac{1}{2}\int \mu^2(x)\pi(x)\,dx - C_1\sqrt{\gamma} > 0$ for sufficiently small $\gamma$. Hence we obtain
\begin{align*}
\sum_{p=j+1}^{q+1} \zeta^{1/4}_{j+1,p}(4) \leq \sum_{p=j+1}^{q+1}e^{4\kappa(p-j)\gamma^2}e^{-2\gamma(p-j)\alpha} < \frac{e^{4\kappa\gamma^2 - 2\gamma\alpha}}{1-e^{4\kappa\gamma^2 - 2\gamma\alpha}}
\end{align*}
and thus the final bound follows:
\begin{align*}
\mathsf{E}\left[\sum_{j=1}^{q}\prod_{k=1}^{j}\alpha_{k}\left(\sum_{p=j+1}^{q+1}\prod_{k=j+1}^{p}\alpha_{k}\right)^{2}\right]  \leq \frac{e^{10\kappa\gamma^2 - 5\gamma\alpha}}{\left(1-e^{4\kappa\gamma^2 - 2\gamma\alpha}\right)^2\left(1-e^{2\kappa\gamma^2 - \gamma\alpha}\right)}
\end{align*}
Plugging into variance bound,
\begin{align*}
\mathsf{Var}_x\left[\sum_{p=1}^{q}\partial_{y_{1}}^{K}f\left(X_{p}\right)\right] \leq
\frac{\gamma^3 A_{K}^{2} e^{10\kappa\gamma^2 - 5\gamma\alpha}}{\left(1-e^{4\kappa\gamma^2 - 2\gamma\alpha}\right)^2\left(1-e^{2\kappa\gamma^2 - \gamma\alpha}\right)(1-\gamma B_\mu)^3} \leq B_K
\end{align*}
for $\gamma$ small enough with $B_K$ not depending on $p,q$. The proof is completed.
\end{proof}
...
We shall show that conditions {\bf (H1)} and {\bf (H2)} are sufficient to show that ULA kernel $Q_\gamma$ satisfies the so-called drift condition:
\begin{definition}
\label{def:drift}
...
\begin{lem}
\label{lem:drift}
 Assume that the potential $U(x), x \in \mathbb{R}^d$ satisfies conditions {\bf (H1)} and {\bf (H2)} and without loss of generality consider $\nabla U(0) = 0$. Then the kernel $Q_\gamma$ from \ref{eq:kernel} satisfies drift condition \ref{def:drift} for any $0 < \gamma < \overline{\gamma} = \frac{m}{4L^2}$ with drift function $V(x) = 1 + \|x\|^2$, constants $\lambda = \exp{\left(-\frac{m}{2}\right)}$, $C = \frac{25K_1^2}{8} + 2d + m$ with $K_1$ from \ref{eq:H3}, $m$ from {\bf (H2)}.
\end{lem}

\begin{proof} Consider $V(x) = 1 + \|x\|^2$, then
$$
Q_{\gamma}V(x) = \int\limits_{\rset^d}V(y)Q_{\gamma}(x,dy) = \int\limits_{\rset^d}(1+\|y\|^2)\frac{1}{(4\pi\gamma)^{\frac{d}{2}}}\exp{\left(-\frac{\|y-x+\gamma\nabla U(x)\|^2}{4\gamma}\right)}\,dy =
$$
