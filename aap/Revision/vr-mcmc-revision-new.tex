...
Let \((X^x_{p})_{p\geq 0}\) be a Markov chain of the form \eqref{eq:ula_new}  and $f$ be a Borel function $\rset^{d}\to\mathbb R$ such that
$\mathsf{E}\left[\left |f(X^x_{p})\right|^{2}\right]<\infty$. Then, for $p>j$, the following representation holds in \(L^2(P)\)
...
Indeed, since $\left(\phi_{k}\right)_{k \geq 0}$ is a complete orthonormal system in \(L^2(\mathbb{R}^d, P_{\xi})\), we have for arbitary $x \in \rset^d$,
