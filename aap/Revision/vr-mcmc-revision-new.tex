...
For any function $f$ such that $\mathsf{E}\left[|f(X^x_{q+1})|^2\right]<\infty$,
the orthonormality and completeness
of the system $\left(\phi_{k}\right)_{k=0}^\infty$ implies that for any $y \in \rset^d$,
$\lim_{n \rightarrow \infty}\Psi_{n,q,1}(y)= 0$ with $\Psi_{n,q,1}(y)= \PE[|f(X^y_{q,q+1}) - f_{n,q,1}(y)|^2]$ and
\[
f_{n,q,1}(y) = \PE[f(X^y_{q,q+1})] +  \sum\limits_{k=1}^{n}\PE[f(X^y_{q,q+1})\phi_{k}(\xi_{q+1})]\phi_k(\xi_{q+1}).
\]
