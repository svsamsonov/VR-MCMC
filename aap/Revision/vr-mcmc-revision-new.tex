...
\widehat{Q}_r = \argmin_{\psi \in \Psi} \sum_{s = 1}^{T} \sum_{l = N + 1}^{N+n_{tr}-r} \left| f(X_{l+r}^{(s)}) - \psi(X_l^{(s)})\right|^2
\end{equation}
for \(r=1,\ldots, n_{tr}-1,\)
where \(\Psi\) is a  class of  real-valued functions on \(\mathbb{R}^d\) and $n_tr$ is a fixed natural number. 
...
=
\frac{1}{n^2}\left[\sum\limits_{k=N+1}^{N+k}\left(f(X_k^x)-\pi(f)\right)\right]^2 \leq \\
&\leq& 
...
\end{equation}
for  $V(x) = 1 + \|x\|^2,$ $\rho\in (0,1)$ specified in Lemma~\ref{13} and constants $C_1,C_2>0$ independent of $\gamma$  and $n$. Since $f$ is bounded, $\|f\|_V \leq \|f\|_{\infty}$. Hence, combining \eqref{eq:bias_1} and \eqref{eq:bias_2}, we obtain \eqref{eq:sq_bias}.
\end{proof}
