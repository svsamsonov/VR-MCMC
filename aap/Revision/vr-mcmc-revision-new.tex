...
Now assume that $j<m$.
The random variable
$\PE \left[\left.f(X^x_{m+1})\right| \mathcal{G}_m \right]$ has the form $g(X^x_{m})$ and is square integrable since 
\[
\PE \left[ \PE \left[\left.f(X^x_{m+1})\right| \mathcal{G}_m \right]^2 \right] \leq \PE \left[ \PE \left[\left.f^2(X^x_{m+1})\right| \mathcal{G}_m \right] \right] = \PE \left[f^2(X^x_{m+1})\right] < \infty
\]
...
\begin{cor}
If  all the kernels $\Phi_l,$ $l\geq 1,$ in \eqref{eq:ula_new} are equal,  then  the representation \eqref{eq:mart_repr} for all $p \in \nset,$ $q \leq p$, $j < q \leq p$, $f \in \mathcal{F}_2^p$ and $x \in \rset^d$ takes the form
\begin{eqnarray*}
...
\begin{prop}
The coefficients \(a_{q,l,k}\) in \eqref{eq:coeff_mart}  can be alternatively represented as
\begin{eqnarray*}
a_{q,l,k}(x)=\mathsf{E}\left[\phi_k\left(\xi\right)Q_{q,l}\left(\Phi_l(x,\xi)\right)\right]
\end{eqnarray*}
with \(Q_{q,l}(x)=\mathsf{E}\left[f(X^x_{l,q})\right],\) \(p\geq l.\)
%where   a r.v. \(\xi\) independent of \(\mathcal{G}_{l-1}.\)
The functions \((Q_{q,l})_{l=0}^p\)  can be computed by the backward recurrence: $Q_{q,q}(x)=f(x)$ and for $l \in \{0,\ldots,q-1\}$
\begin{eqnarray}
\label{eq:qpl}
...
\\
&=& \sum_{k=1}^{K}\sum_{l=N+1}^{N+n}\left(1+\frac{N-l+1}{n}\right)A_{N+n-l,k}(X^x_{l-1})\phi_{k}\left(\xi_{l}\right),
\label{eq:29032018a5}
\end{eqnarray}
where
\begin{equation}
