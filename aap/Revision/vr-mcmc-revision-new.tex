...
\end{eqnarray}
where $F_s(x,Z_1,\ldots,Z_s)=\sum_{r=0}^s f(X^x_r)$ and $\partial_{Z_1}^{\mathbf{k}'}$
...
\PVar_x \left( \sum_{p=1}^{s}\partial_{Z_{1}}^{\mathbf{K}_I}f\left(X_{p}\right)
\right),
\end{eqnarray*}
...
\right)\leq C(1+V(x))
\end{equation*}
for some constant $C$ not depending on $q$ and $V(x) = 1 + \|x\|^2$.
