...
\end{equation}
Note that $\left(X^x_{0,p}\right)_{p \in \nset_0}$ is a Markov chain with values in $\rset^d$ of the form \eqref{eq:chain_gen}, starting at $X_0 = x$. We write $X^x_{p}$ and $G_{p}$ as a shorthand notation for $X^x_{0,p}$ and $G_{0,p}$, respectively. Let us also define by $\mathcal{F}_2^p$ class of Borel functions $f: \rset^d \to \rset$ such that for any $1 \leq l \leq q \leq p$ and $y \in \rset^d$, $\mathsf{E}\left[\left |f(X^y_{l,q})\right|^{2}\right]<\infty$. Note that for $q \leq p$ it holds $\mathcal{F}_2^p \subseteq \mathcal{F}_2^q$.

\begin{thm}\label{prop:29032018a1}
For all $p \in \nset,$ $q \leq p$, $j < q \leq p$, $f \in \mathcal{F}_2^p$ and $x \in \rset^d$ the following representation holds in \(L^2(\mathsf{P})\)
\begin{eqnarray}
\label{eq:mart_repr}
...
a_{q,l,k}(y)=\mathsf{E}\left[f(X^y_{l-1,q})\phi_k\left(\xi_{l}\right)\right], \quad q\geq l, \quad k\in \mathbb{N}
\end{eqnarray}
\end{thm}
...
\]
for any $f \in \mathcal{F}_2^p$  with $a_{1,1,k}(x)=\PE[f(X_{1}^x)\phi_{k}(\xi_{1})]$.
Assume now that (\ref{eq:mart_repr}) holds for some $m < p$, any $q \leq m$, $j < q \leq m$ and $f$ from $\mathcal{F}_2^p$. Let us prove that the induction assumption holds for $q=m+1$ and all $j < q$.
For any function $f$ from $\mathcal{F}_2^p$,
the orthonormality and completeness
of the system $\left(\phi_{k}\right)_{k=0}^\infty$ implies that for any $y \in \rset^d$, 
...
\]
Note that  for any $y \in \rset^d$ and $n \in \nset$ it holds $\Psi_{n,m+1,1}(y) \leq \Psi_{0,m+1,1}(y)$ and hence
\begin{eqnarray*}
\PE \psi_{0,m+1,1}(X_m^x) &=& \PE \left[ \left. \PE \left[ \psi_{0,m+1,1}(X_m^x) \right| \mathcal{G}_m\right]\right] = \PE \left[ \left. \PE \left[ |f(X^{X_m^x}_{m,m+1}) - \PE f(X^{X_m^x}_{m,m+1})|^2 \right| X_m^x\right]\right]
\\
&=& \PE \left[ |f(X_{m+1}^x - \PE f(X_{m+1}^x))|^2 \right] = \PVar{f(X_{m+1}^x)} < \infty
\end{eqnarray*}
Hence, by Lebesgue dominated convergence theorem, $\PE \psi_{n,m+1,1}(X_m^x) \rightarrow 0$ and since for all $y \in \rset^d$ the expectation $\PE[f(X^y_{m,m+1})]$ is a version of $\PE \left[f(X^y_{m,m+1}) | \mathcal{G}_m \right]$, it holds in \(L^2(\mathsf{P})\) that
\begin{equation}
...
Now assume that $j<m$.
The random variable
$\mathsf{E}\left[\left.f(X^x_{m+1})\right|X^x_{m}\right]$
