...
\erici{the notation $\partial^{\mathbf{k}}$ should be defined here}
...
\begin{equation}
\label{eq:smooth-mu}
|\partial^{\mathbf{k}} f(x)|\le  B_f, \quad |\partial^{\mathbf{k}} U (x)|\leq B_\mu, \quad
x\in\mathbb R^d
\end{equation}
\eric{I have replaced $\mu$ by $U$; in this section we use mostly $U$ and not $\mu$; we could use $\mu$ but in such case it would be better to recall that $\mu= \nabla U/2$}
\eric{Is it required to assume that the gradient is bounded ? It contradicts assumption (H2) which assumes a quadratic growth at infinity - coercivity of the operator ?}
