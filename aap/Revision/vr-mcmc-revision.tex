...
\begin{lem} Assume that the potential $U(x), x \in \mathbb{R}^d$ satisfies conditions {\bf (H1)} and {\bf (H2)} and without loss of generality consider $\nabla U(0) = 0$. Then the kernel $R_\gamma$ from \ref{eq:kernel} satisfies drift condition \ref{def:drift} for any $0 < \gamma < \overline{\gamma} = \frac{m}{4L^2}$ with drift function $V(x) = 1 + \|x\|^2$, constants $\lambda = \exp{\left(-\frac{m}{2}\right)}$, $C = \frac{25K_1^2}{8} + 2d + m$ where $K_1$ is from \ref{eq:H3}, $m$ is from {\bf (H2)}. 
\end{lem}
...
For now let us assume for simplicity that we run ULA with fixed step size $\gamma$. Then it is known that under assumption {\bf H1} corresponding Markov chain would have unique stationary distribution $\pi_\gamma$, which is not equal to $\pi$. Yet this chain will be $V-$geometrically ergodic due to \cite[Theorem~19.4.1]{douc:moulines:priouret:soulier:2018}. Namely, the following lemma holds:

\begin{lem} Assume that the potential $U(x), x \in \mathbb{R}^d$ satisfies conditions {\bf H1} and {\bf H2}. Then for $0 < \gamma < \overline{\gamma}=\frac{m}{4L^2}$, for any $x \in X$ it holds
$$
...
$$
\rho = \exp^{-\gamma\left(\frac{m}{2} - 2\frac{\frac{25K_1^2}{8} + 2d + m}{d}\right)\frac{\log{(1-\varepsilon)}}{\log{(1-\varepsilon)} + \log{(\exp^{-\frac{m\gamma}{2}} + \frac{2b}{d+1})}}}
$$
\end{lem}
