...
%\som{Changed exponent from 1/2 to 2 here}
...
\]
with $\Gamma^* = \sum\limits_{k=1}^{\infty}\gamma_k^2, \kappa = ..., \Gamma_{l,j} = \sum\limits_{k=l}^{j}\gamma_k, \alpha = \int \mu^2(x)\pi(x)\,dx$. Since $\sum\limits_{k=1}^{\infty}\gamma_k = \infty$, we may bound
\begin{align*}
...
Due to \cite[Lemma~14]{brosse:2018:diffusion}, it is known that conditions (H1) and (H2) imply the result

\begin{description}
\item[{\bf (H3)}] There exist $K_1 > 0, m > 0$, such that for any $x \not\in B_{K_1}(0)$ it holds
\begin{eqnarray*}
\langle \nabla U(x), x \rangle \geq \frac{m}{2}\|x\|^2
\end{eqnarray*}
\end{description}

\begin{lem} Assume that the potential $U(x), x \in \mathbb{R}^D$ satisfies conditions {\bf (H1)} and {\bf (H3)} and without loss of generality consider $\nabla U(0) = 0$. Then the kernel \ref{} satisfies drift condition \ref{eq:drift} for any $0 < \gamma < \overline{\gamma} = \frac{m}{4L^2}$ with drift function $V(x) = 1 + \|x\|^2$, constants $\lambda = \exp{-\left(\frac{m}{2}\right)}$, $C = 3K_1^2 + 2D + C_1$ for some absolute constant $C_1 > 0$, , which does not depend on $\gamma, D, x$.
\end{lem}
