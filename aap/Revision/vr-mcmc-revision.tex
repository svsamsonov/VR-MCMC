...
Suppose that
\begin{eqnarray*}
\sum_{p=1}^{\infty} \gamma^2_p<\infty.
\end{eqnarray*}
Under assumptions (H1) and (H2), then for any bounded function $f$ on $\mathbb{R}^d$ 
we have
\begin{eqnarray*}
\mathsf{E}\left[\left(\sum_{p=l}^{N+n}\omega_{p,n}^{N}f(X_{p})-\bar Q_l(X_l)\right)^{4}\right] \lesssim \frac{1}{\Gamma^4_{N+2,N+n+1}},
\end{eqnarray*}
 and
\begin{eqnarray*}
