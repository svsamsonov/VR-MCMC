...
\begin{eqnarray*}
\Phi_p(x,(u,z)^\top)=x+\mathbbm{1}\bigl(u\leq \alpha(x,x-\gamma_{p}\mu(x)+\sqrt{2\gamma_{p}}z)\bigr)(-\gamma_{p}\mu(x)+\sqrt{2\gamma_{p}}z).
\end{eqnarray*}
\end{example}
\begin{example}
...
In this section we perform the convergence analysis of the ULA algorithm. For the sake of simplicity we restrict our attention to the constant time step.
...
\sum_{p=N+1}^{N+n}
\omega_{p,n}^N\,
\|\delta_x Q_\gamma^{p}-\pi(\cdot)\|_{TV}^2,
\notag
\end{align}
where
...
is defined by
$$
\mathcal{R}_\gamma(x,A)=\int_{A} \frac{1}{(4 \pi \gamma)^{d/2}} \text{ exp} \left\{ -\frac{1}{4\gamma} \left\| y - x + \gamma \mu(x)\right\|^2\right\}dy,
$$
while, for $k,l\in\mathbb N$, $k\le l$,
the kernel $\mathcal{Q}_{\boldsymbol{\gamma}}^{k,l}$ is
$
\mathcal{Q}_{\boldsymbol{\gamma}}^{k,l}=\mathcal{R}_{\gamma_l}\cdots \mathcal{R}_{\gamma_k},
$
which, finally, provides the  measure
$\mathcal{Q}_{\boldsymbol{\gamma}}^{p}=\mathcal{Q}_{\boldsymbol{\gamma}}^{p,1}$
...
on bounds in the total variation distance. For the case of a constant time step $\gamma_1=\ldots=\gamma_p=\gamma$ it is known that under assumption {\bf (H1)} the corresponding Markov chain has a unique stationary distribution $\pi_\gamma$, which is different from $\pi$. Moreover, we shall show in Section that  under {\bf (H1)} and {\bf (H2)}
\begin{eqnarray}
\label{eq:bias}
\|\delta_x\mathcal{Q}_{\boldsymbol{\gamma}}^{n}-\pi_\gamma\|_{TV} \leq C\rho^{\gamma n}\left(V(x) + \pi_\gamma(V)\right)
\end{eqnarray}
for  $V(x) = 1 + \|x\|^2,$ some $\rho\in (0,1)$ independent of $\gamma$ and an absolute constant $C>0.$
\subsection{Conditional variance}
An upper bound for the variance of the classical estimator~\eqref{eq:29032018a2}
under {\bf (H1)} and {\bf (H2)} is derived in Section.  In particular, if $\sum_{j=1}^\infty \gamma_j=\infty$ and $\sum_{j=1}^\infty \gamma^2_j<\infty,$ then for any bounded function $f,$
\begin{eqnarray}
\label{eq:var-mc}
\mathsf{Var}[\pi_n^N(f)]\lesssim 1/\Gamma_{N+2,N+n+1},
\end{eqnarray}
where $\lesssim$ stands for inequality up to a constant depending on constants $L_U,$ $m_U,$  $M_U$ and $\|f\|_\infty.$ For a constant time step,  we derive from \eqref{eq:var-mc} that
$\mathsf{Var}[\pi_n^N(f)]\lesssim 1/(n\gamma).$
...
\end{equation}
\end{prop}
One of the main results of this paper is the following  upper bound for the right-hand sides of~\eqref{eq:29032018a6} in the case of ULA algorithm.
...
\end{eqnarray*}
Obviously \(\mathsf{E}[\widehat M_{K,n}^N(f)|\TrainSet]=0\) and \(\widehat M_{K,n}^N(f)\) is indeed a valid control variate in a sense that it
...
In this appendix we give some definitions and prove auxiliary results which are the used to bound the exponential moment of the additive functional of ULA chain (Theorem~\ref{th:exp_mom_ula}).
\par
For $\gamma>0$, the Markov kernel $R_\gamma$ on $(\mathbb R^d,\mathcal B(\mathbb R^d))$, corresponding to the Unadjusted Langevin Algorithm with step $\gamma$ is defined by
\begin{equation}
\label{eq:kernel}
...
Due to \cite[Lemma~14]{brosse:2018:diffusion}, conditions {\bf (H1)} and {\bf (H2)} implies that there exist constants $K_1 > 0, m > 0$, such that for any $x \not\in B_{K_1}(0)$ it holds
\begin{equation}
\label{eq:H3}
\langle \nabla U(x), x \rangle \geq \frac{m}{2}\|x\|^2
\end{equation}

We would show that conditions {\bf (H1)} and {\bf (H2)} are sufficient to show that ULA kernel $R_\gamma$ satisfies the so-called drift condition:
\begin{definition}
\label{def:drift}
...
\begin{eqnarray*}
d_V(\delta_xQ_{\boldsymbol{\gamma}}^{1,n},\pi) \leq K_3 V(x) \rho^{\Gamma_{1,n}} 
\end{eqnarray*}
\end{description}
