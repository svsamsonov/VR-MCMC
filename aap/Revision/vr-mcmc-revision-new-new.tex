...
\]
Note that $\mu^{\prime}(x)$ is bounded and Lipschitz due  our assumptions and from Lemma~\ref{lem:exp_bound} in Appendix~\ref{sec:appendix} it follows that
\begin{align*}
\zeta_{l,j}(u) \leq \begin{cases} C(x)\exp\left\{(j-l)\gamma\left(4u^2\kappa^{-2} \|f\|^2_{\text{Lip}} + M^2 + C_1u\sqrt{\gamma} -  u\pi(\mu')\right)\right\}, \quad \gamma(j-l) \geq C_0 := \left(\frac{4}{m+L} + \kappa^{-1}\right) \\
\exp\left\{uC_0\max\limits_{y}|\mu'(y)|\right\}, \quad 0 \leq \gamma(j-l) \leq C_0	
\end{cases}
\end{align*}
for constants $C(x)$ and $C_1$ which could be made precise. Furthermore, since $\mu\pi',\mu'\pi\in L^{1}(\mathbb{R})$ and $\pi^{\prime}(x) = -\frac{1}{2}\pi(x)\mu(x)$, we have
\[
...
\]
 with $0 < \gamma < \frac{4}{m+L}, \quad \mu(x) = \mu_{0}(x) + \Delta(x)$, where $\mu_0^{\prime}(x) <-m$ for some $m > 0$, $\mu_0(x) \in Lip(L)$. Suppose that there exists $M > 0,$ such that $\left|\Delta(x)\right| \leq M$.
 Then for any Lipschitz function $f$, $\forall N $
\begin{equation}
\label{eq:exp_bound}
\PE_x \rme^{u\gamma\sum\limits_{k=N+1}^{N+n}f(X_k)} \leq C\exp\left\{n\gamma\left(2u^2\kappa^{-2} \|f\|^2_{\text{Lip}}\left(1+\frac{1}{n\gamma}\left(\frac{4}{m+L} + \kappa^{-1}\right)\right) + M^2 + C_1u\sqrt{\gamma} +  u\pi(f)\right)\right\}
\end{equation}
with positive constants $C < \exp\left\{\frac{4u\|f\|_{\text{Lip}}}{mL}\int_{\rset}|x-y|\,\pi_{\gamma}(dy)\right\}$ and $C_1$ depending only on $m,L$.
\end{lem}
