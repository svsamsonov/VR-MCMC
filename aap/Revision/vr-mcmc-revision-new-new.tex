...
$\bar a_{r,k}(y)=\mathsf{E}\left[f(X^y_{r})\phi_k\left(\xi_{1}\right)\right]$, $r, k\in \mathbb{N}$.
\end{cor}
...
Let $f$ be a bounded Borel function and $(X^x_{p})_{p \in \nset_0}$ be a Markov chain generated by ULA with constant step size $\gamma \in (0,\gamma_0]$ where $\gamma_0= m/L^2$. 
Let $c_0>0$. Then, for all $n \in \nset$, $\gamma \in (0,\gamma_0]$ satisfying $n\gamma > c_0$   
\begin{equation}
\label{eq:var-mc}
\PVar[\pi_n^N(f)]\lesssim \frac{V(x)}{n\gamma} \eqsp,
\end{equation}
where $\lesssim$ stands for inequality up to a constant not depending on $\gamma \in (0,\gamma_0]$ and $n \in \nset$.
\end{lem}
\begin{remark}
\label{rem:var-low}
...
\end{equation}
Then, it holds that for $q=0,\ldots,n-1,$ and all $x$
\begin{eqnarray}
\label{eq:Aqk2}
A^2_{q,\mathbf{k}}(x)\leq C_K \rme^{\varkappa n\gamma^2}(1+V(x)),\quad 1\leq |\mathbf{k}|\leq K,
\end{eqnarray}
 and
\begin{eqnarray}
\label{eq:Aqk2sum}
