...
\]
Note that due to our assumptios, $\mu(x) = \mu_0(x) + \Delta(x)$ with $\mu^{\prime}_0(x) < -m$. Thus by applying Holder inequality to $\zeta_{l,j}(u)$, we obtain
\begin{equation}
\label{eq:zeta}
\zeta_{l,j}(u) \leq \left(\mathsf{E}\left[\rme^{-2u\gamma\sum_{k=l}^{j}\mu_0'(X^x_{k-1})}\right]\right)^{1/2}\left(\mathsf{E}\left[\rme^{-2u\gamma\sum_{k=l}^{j}\Delta'(X^x_{k-1})}\right]\right)^{1/2}
\end{equation}
Note that $\left|\Delta^{\prime}(x)\right| \leq M^{\prime}$, hence
\[
\left(\mathsf{E}\left[\rme^{-2u\gamma\sum_{k=l}^{j}\Delta'(X^x_{k-1})}\right]\right)^{1/2} \leq \mathsf{E}\rme^{2u(j-l)M^{\prime}}
\]
...
\begin{align*}
\zeta_{l,j}(u) \leq \begin{cases} C(x)\exp\left\{(j-l)\gamma\left(4u^2\kappa^{-2} \|\mu^{\prime}\|^2_{\text{Lip}} + M^2 + M^{\prime} + C_1u\sqrt{\gamma} -  u\pi(\mu')\right)\right\}, \quad \gamma(j-l) \geq C_0 \\
\exp\left\{uC_0\max\limits_{y}|\mu'(y)|\right\}, \quad 0 \leq \gamma(j-l) \leq C_0	
\end{cases}
\end{align*}
with $C_0 = \left(\frac{4}{m+L} + \kappa^{-1}\right)$, and constants $C(x)$ and $C_1$ given in Lemma~\ref{lem:exp_bound} . Furthermore, since $\mu\pi',\mu'\pi\in L^{1}(\mathbb{R})$ and $\pi^{\prime}(x) = -\frac{1}{2}\pi(x)\mu(x)$, we have
\[
...
\label{lem:v_ergodicity}
 Assume  {\bf (H1)} and {\bf (H2)}. Then for $0 < \gamma < \overline{\gamma}= \min(m / 4L^2,1/L)$, for $x \in \rset^d: \|x\| \leq K$  it holds
$$
...
$$
b = \gamma(\frac{25K_1^2}{8} + 2d + m); \quad \overline{b} = b\exp{\left(-\frac{m\gamma}{2}\right)} + K^2 + 1; \quad \varepsilon = 2\Phi\left(-K\sqrt{\left(1+\frac{1}{L}\right)\left(1 + 2L\right)}\right); \quad
$$

$$
\rho = \exp{\left(-\gamma\left(\frac{m}{2} - 2\frac{\frac{25K_1^2}{8} + 2d + m}{K^2 + 1}\right)\frac{\log{(1-\varepsilon)}}{\log{(1-\varepsilon)} + \log{(\exp{\left(-\frac{m\gamma}{2}\right)} + \frac{2b}{K^2+2})}}\right)}
$$
\end{lem}
...
\]
with $\varepsilon = 2\Phi\left(-K\sqrt{\left(1+\frac{1}{L}\right)\left(1 + 2L\right)}\right)$ for all $x,y$ such that $\|x\|,\|y\| \leq K$, which means that the kernel $Q_{\gamma}$ satisfies $([1/\gamma],\varepsilon)$-Doeblin codition on the set $\{x: V(x) < 1+K^2\}$. Together with drift condition \eqref{eq:lyapunov} it allows to apply \cite[Theorem~19.4.1]{moulines2018} with appropriate constants.
\end{proof}
