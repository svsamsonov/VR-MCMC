...
We say that the Markov chain $(X_p)_{p \in \nset_0}$  taking values in some space $X$ with the Markov kernel $Q$ is $V-$geometrically ergodic for a given function $V: X \rightarrow [1; +\infty)$ if there exist real numbers $C > 0$ and $0 < \rho < 1$ such that for any $n \in \nset$,
\begin{equation}
\label{eq:v_ergodicity}
d_V(\delta_xQ^{1,n},\pi_\gamma) \leq C\rho^n V(x)
\end{equation}
\end{definition}
...
Now we shall show that conditions {\bf (H1)} and {\bf (H2)} are sufficient to show that ULA kernel $Q_\gamma$ satisfies the drift condition \eqref{eq:drift}
