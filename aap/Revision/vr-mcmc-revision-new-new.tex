...
\end{equation}
with constant $C$ from Lemma~\ref{eq:fp_cov}.
\end{lem}
\begin{proof}
Note that
\[
...
\leq \{\eqref{eq:fp_cov}\}\leq C\rho^{s/2}\|\tilde{f}\|_{V^{\frac{1}{2}}}\int\limits_{\rset^d} V(y) |\P^k(x,dy)-\pi(dy)| \leq 
\\
\leq C^2\rho^{s/2}\|\tilde{f}\|_{V^{\frac{1}{2}}}\|\P^k(x,dy)-\pi(dy)\|_{V} \leq C^2\rho^{k+s/2}\|\tilde{f}\|_{V^{\frac{1}{2}}}V(x)
\end{multline*}
