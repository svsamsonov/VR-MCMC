...
\widetilde{A}_{q,k} \in \argmin_{\psi \in \Psi} \sum_{s = 1}^{T} \left| \sum_{r = 0}^{q}  f(X_{r+L}^{(s)})\phi_k(\xi^{(s)}_{L+1}) - \psi(X^{(s)}_L)\right|^2
\end{equation}
...
\begin{eqnarray*}
\PE[A^2_{l,k}(X^x_{N+l-1})]\leq R^2_k,\quad k=1,2,\ldots, 
\end{eqnarray*}
for a sequence  $(R_k)$ satisfying $\sum_{k=1}^\infty R^2_k\leq C$ with constant $C$  depending neither on $\gamma$ nor on $n$ (see \eqref{eq:Aqk2}). Hence if we take $U_k=R_k$ as truncation parameters in \eqref{eq:trunc}, then $\widehat A_{n-l,k}$ converges to $A_{n-l,k}$ as $T\to \infty.$ Moreover,
\begin{eqnarray*}
\PVar[\widehat \pi_{K,n}^N(f)|\TrainSet]\leq \frac{2C}{n}, 
\end{eqnarray*}
for a constant $C$ not depending on $n$ and $\gamma.$
If the class of functions $\Psi$ is linear of dimension $\mathrm{D}_\Psi$ As can be easily seen the cost of computing $\widehat \pi_{K,n}^N(f)$ is of order $ K T^2 n$. At the same time the variance of the standard estimate $\widehat \pi_{n}^N(f)$ is of order $1/(n\gamma)$ and this bound can not be improved in general. Thus the ratio of cost-variances is of order
\begin{eqnarray*}
\frac{\mathrm{cost}(\pi_{K,n}^N) \PVar[\widehat \pi_{K,n}^N(f)|\TrainSet]}{\mathrm{cost}(\pi_{n}^N)\PVar[\pi_{n}^N(f)]}\leq C \gamma K T^2 L^2 \mathrm{Dim}(\Psi). 
\end{eqnarray*}
If $K,$ $L$ and $\mathrm{Dim}(\Psi)$ are fixed, then our variance reduction method is advantageous if $\gamma=O(1/T^2).$ 
...
\end{equation}
Suppose that $n\gamma^2\leq \Gamma.$ Set $V(x) = 1 + \|x\|^2,$ then  it holds
\begin{eqnarray}
\label{eq:Aqk2}
\sum_{\mathbf{k}\colon\|\mathbf{k}\|\geq K} A^2_{q,\mathbf{k}}(x)\leq C_{K}\sum_{I\subseteq\{1,\ldots,d\},\, I\neq \emptyset}
\left(\frac{\gamma}{2}\right)^{|I|K-1}
(1+V(x))
\end{eqnarray}
for some constant $C_K$ not depending on $q$ and depending on $\gamma$ only through $\Gamma$. 
\end{thm}
\begin{cor}
...
\eqsp,
\end{eqnarray}
where $\lesssim$ stands for inequality up to a constant depending only on $K$ and $\Gamma$.
\end{cor}
