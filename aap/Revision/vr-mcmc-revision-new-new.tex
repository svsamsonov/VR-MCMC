...
\end{eqnarray}
where $\rho \in (0,1)$, $\mu \in \rset^2$, $\Sigma$ is a symmetric positive definite $2 \times 2$ matrix and $|\Sigma|$ is its determinant. The function $U(x)$ and its gradient are given by
\[
...
\label{lem:drift}
 ...
 Then the kernel $Q_\gamma$ from \eqref{eq:ula_kernel} satisfies drift condition \eqref{eq:lyapunov} for any $0 < \gamma < \overline{\gamma} = \frac{m}{4L}$ with drift function $V(x) = 1 + \|x\|^2$,   $\lambda = \exp{\left(-\frac{m}{2}\right)}$, $C = \frac{25K_2^2}{8} + d + m$ with $K_2$ from \eqref{eq:H3}, $m$ from {\bf (H2)}.
\end{lem}
