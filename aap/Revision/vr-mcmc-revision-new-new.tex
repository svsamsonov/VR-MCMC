...
$$
C = \left(1+\pi_\gamma(V)\right)\left(1 + \exp{\left(-\frac{m\gamma}{2}\right)}\right)\left(1+\frac{b\exp{\left(-\frac{m\gamma}{2}\right)} + K^2 + 1}{(1-\varepsilon)(1-\exp{\left(-\frac{m\gamma}{2}\right)} - \frac{2b}{2+K^2})}\right);
$$

$$
b = \gamma\left(2K_2^2(L+m) + d + m\right); \quad \varepsilon = 2\Phi\left(-K\sqrt{\left(1+\frac{1}{L}\right)\left(1 + 2L\right)}\right); \quad
$$

$$
\rho = \exp\left\{-\frac{\gamma m}{4}\frac{\log{(1-\varepsilon)}}{\log{(1-\varepsilon)} + \log{(\exp{\left(-\frac{m\gamma}{2}\right)} + \frac{2b}{K^2+2})}}\right\}
$$
...
\PVar_x \left[ \pi_{N}^n(f) \right] \leq \frac{C_1\pi(V)\|\tilde{f}\|^2_{V^{1/2}}}{n(1-\rho^{1/2})} + C_2\left(\frac{\rho^NV(x) + 1}{n(1-\rho^{1/2})}\right)^2
\end{equation}
with positive constants $C_1,C_2 > 0$ and $0 < \rho <1$ from \eqref{eq:v_ergodicity}.
\end{lem}
